...
struct FieldMatrixHelper
{
  ...
  //! Compute A*x and store the result int ret
  ...
  //! Compute A^T*x and store the result int ret
  template< int m, int n >
  ...
  //! Compute A*B and store the result int ret
  ...
  //! Compute A^T*B^T and store the result int ret
  template< int m, int n, int p >
  ...
  //! Compute A^T*A and store the lower triangular part int ret
  template< int m, int n >
  ...
  //! Compute A^T*A and store the result int ret
  template< int m, int n >
  ...
  //! Compute A*A^T and store the lower triangular part int ret
  template< int m, int n >
  ...
  //! Compute A*A^T and store the result int ret
  template< int m, int n >
  ...
  static ctype spdDetA ( const FieldMatrix< ctype, n, n > &A )
  {
    ...
    else if (n == 3)
    {
      ...
      ctype t4  = A[0][0] * A[1][1] * A[2][2];
      ctype t6  = A[0][0] * A[1][2] * A[1][2];
      ctype t8  = A[0][1] * A[0][1] * A[2][2];
      ctype t10 = A[0][1] * A[0][2] * A[1][2];
      ctype t14 = A[0][2] * A[0][2] * A[1][1];
      return (t4-t6-t8+2*t10-t14);
    }
    else {
  ...
  static ctype leftInvA ( const FieldMatrix< ctype, m, n > &A, FieldMatrix< ctype, n, m > &ret )
  {
    ...
    if( (n == m) && (n <= 3) )
      return FMatrixHelp::invertMatrix(A,ret);
    else
    {
      FieldMatrix< ctype, n, n > ata;
      ATA_L( A, ata );
